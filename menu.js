/*
 ...
 | Created: Jul. 19, 2002                Last modified: May 25, 2005 |
 +-------------------------------------------------------------------+
...
//----------------------------------------------------------------------------------------------------
  ...
  this.fadeOutDelay = 1000;                  // fade-out delay (1/1000 seconds)
  ...
  this.mainTop = 123;                         // top position (pixels)
  this.mainLeft = 0;                        // left position (pixels)
  this.mainBGColor = "#333333";             // background color OR background image
  this.mainBorderWidth = 1;                // border width (pixels)
  ...
  this.mainItemColor = "#333333";           // item color
  this.mainItemHilight = "#80A0C0";         // item hilight color
  this.mainItemFont = "Verdana, Arial, Helvetica";   // font family (CSS-spec)
  this.mainItemFontColor = "white";         // font color
  this.mainItemFontSize = 10;               // font size (pixels)
  this.mainItemAlign = "center";            // text align (left / center / right)
  this.mainItemPadding = 1;                 // text padding (pixels)
  ...
  this.subItemWidth = 150;                  // item width (pixels)
  ...
  this.subItemFontSize = 10;                // font size (pixels)
...
//----------------------------------------------------------------------------------------------------
  ...
  this.entry = function(level, height, text, url, onClick) {
    var nr = this.items.length;
    var parent = nr - 1;
    this.items[nr] = new makeItem(nr, level, height, text, url, onClick);
    if(parent >= 0) {
  ...
  this.buildItems = function(parent, section) {
    ...
    for(var i = cnt = 0; i < this.items.length; i++) {
      if(this.items[i].parent == parent) {
        ...
        if(!topY) topY = border;
        link = "javascript:mObj[" + this.mNr + "].jumpURL('" + (this.items[i].url ? this.items[i].url : '') + "')";
  ...
  this.jumpTo = function() {
    ...
    if(item) {
      ...
      this.jumpURL(this.items[item].url);
    }
  }
  //-------------------------------------------------------------------------------------
}
